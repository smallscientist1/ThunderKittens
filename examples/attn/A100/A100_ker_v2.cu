...
__global__ void attend_ker64(int n, const bf16* __restrict__ __q__, const bf16* __restrict__ __k__, const bf16* __restrict__ __v__, bf16* __o__) {
    ...
    st_bf<NUM_WORKERS,4,ducks::st_layout::swizzle> (&k_smem) = al.allocate<st_bf<NUM_WORKERS,4,ducks::st_layout::swizzle>>();
    st_bf<NUM_WORKERS,4,ducks::st_layout::swizzle> (&v_smem) = al.allocate<st_bf<NUM_WORKERS,4,ducks::st_layout::swizzle>>();
    ...
    rt_bf<1,4> q_reg;
    rt_bf<NUM_WORKERS,4> k_reg, v_reg; // v_reg need to be swapped into col_l
    rt_fl<1,NUM_WORKERS> att_block;
    rt_bf<1,NUM_WORKERS> att_block_mma;
    rt_fl_1x4<> o_reg;
    ...
    // for(auto q_blk = 0; q_blk < qo_blocks; q_blk++) {
        ...
        for(auto kv_idx = 0; kv_idx < kv_blocks; kv_idx++) {
            ...
            load(v_smem, _v + (kv_idx*NUM_WORKERS )*q_reg.num_elements, q_reg.cols, warpid, 1);
            load(k_smem, _k + (kv_idx*NUM_WORKERS )*q_reg.num_elements, q_reg.cols, warpid, 1);
            ...
            // for(int subtile = 0; subtile < NUM_WORKERS; subtile++) {
                ...
                load(k_reg, k_smem); // load k from shared into registers

                zero(att_block); // zero 16x64 attention tile
                ...
                load(v_reg, v_smem); // load v from shared into registers.
                rt_bf<NUM_WORKERS,4,ducks::rt_layout::col> &v_reg_col = swap_layout_inplace(v_reg); // this is a reference and the call has invalidated v_reg
